 ...
 * model.
 */
public class LabelImageUtil {
	/**
	 * tensorflow_inception_graph，imagenet_comp_graph_label_strings文件路径
	 */
	private static final String MODELDIR = "E:\\tensorflowFile";
	 ...
	 * @return
	 */
	public static String parseImage(String imageFile) {
		String imageName = "";
		byte[] graphDef = readAllBytesOrExit(Paths.get(MODELDIR, "tensorflow_inception_graph.pb"));// 读二进制pb文件
		List<String> labels = readAllLinesOrExit(Paths.get(MODELDIR, "imagenet_comp_graph_label_strings.txt"));// 读label文件
		byte[] imageBytes = readAllBytesOrExit(Paths.get(imageFile));// 图片-->字节型数组
		try {
			Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes);
			float[] labelProbabilities = executeInceptionGraph(graphDef, image);
			int bestLabelIdx = maxIndex(labelProbabilities);
			imageName = labels.get(bestLabelIdx);
			//System.out.println(String.format("最佳匹配: %s (%.2f%% likely)", labels.get(bestLabelIdx),
			//		labelProbabilities[bestLabelIdx] * 100f));
		}catch (Exception e) {
			System.out.println("解析图片异常"+e.getStackTrace());
		}
		return imageName;
	}
	...
	private static Tensor<Float> constructAndExecuteGraphToNormalizeImage(byte[] imageBytes) {
		try (Graph g = new Graph()) {
			...
			final float scale = 1f;// 1f change 117f
